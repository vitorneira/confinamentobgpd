...
const ICONE: Record<Alerta["severidade"], string> = { atencao: "▲", critico: "■" };
...
export function AlertList({ alertas }: { alertas: Alerta[] }) {
  if (alertas.length === 0) {
    return (
      <p className="rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-800 dark:border-green-900 dark:bg-green-950 dark:text-green-400">
  ...
  return (
    <ul className="space-y-2">
      {alertas.map((a, i) => {
        const conteudo = (
          <div className={`flex items-center gap-3 rounded-lg border px-4 py-3 text-sm ${COR[a.severidade]}`}>
            <span aria-hidden>{ICONE[a.severidade]}</span>
